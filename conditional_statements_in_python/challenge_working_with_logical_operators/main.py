...
can_bob_and_john_enter = bool(bobs_age >= adult_age) and (johns_age >= adult_age)

# Checking if either Alice or Mike can enter the park (at least one must be an adult)
can_alice_or_mike_enter = bool(alices_age >= adult_age) or (mikes_age >= adult_age)

# Checking if Bob, Alice, and John can all enter the park together (all must be adults)
can_bob_alice_john_enter = bool(bobs_age and alices_age and johns_age >= adult_age)
